    ...
    "Data": [
      ...
      },
      {
      ...
      },
       {
        ...
        "Designation": "Employee",
        "Reviewer": "T.Pardhasaradhi",
        "Email ID": "merugu.akhila@showtimeconsulting.in",
        "Emp code":"STC25077"
...
export const generateChannelsFromDepartments = () => {
  ...
  // Add department channels with 'dept-' prefix to match backend expectations
  Object.keys(DEPARTMENT_DATA).forEach(dept => {
    channels.push({
      id: channelId++,
      name: `dept-${dept.toLowerCase().replace(/ /g, '-')}`,
      type: "department",
      department: dept,
    ...
    Object.keys(DEPARTMENT_DATA[dept]).forEach(subDept => {
      if (subDept !== dept) { // Avoid duplicate names
        channels.push({
          id: channelId++,
          name: `team-${subDept.toLowerCase().replace(/ /g, '-')}`,
          type: "team",
          department: dept,
