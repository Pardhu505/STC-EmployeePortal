...
/* winner artwork supplied per rank (transparent PNGs) */
const ART = { 1: '/festive/winner1.png', 2: '/festive/winner2.png', 3: '/festive/winner3.png' };
          ...
          {order.map((rank) => {
            ...
            return (
              <div key={rank} className={`gw-slot gw-r${rank}`}>
                ...
                <img className="gw-fig" src={ART[rank]} alt={w.name}
...
const CSS = `
.gw-overlay{position:fixed;inset:0;z-index:10040;overflow-y:auto;overflow-x:hidden;
  display:flex;align-items:flex-start;justify-content:center;padding:14px 16px 28px;
...
.gw-inner{position:relative;z-index:4;text-align:center;max-width:1000px;width:100%;
  margin:auto;padding:6px 0 10px;}
.gw-om{font-size:22px;color:#ffd77a;text-shadow:0 2px 10px rgba(0,0,0,.7);}
.gw-title{margin:0;font-size:clamp(22px,4vw,38px);font-weight:900;color:#fff3d6;
...
.gw-bless{max-width:600px;margin:8px auto 0;color:#f6e4c4;font-size:13px;line-height:1.55;
  text-shadow:0 2px 6px rgba(0,0,0,.7);}
.gw-bless b{color:#ffd77a;}
.gw-podiums{display:flex;align-items:flex-end;justify-content:center;gap:clamp(8px,2.4vw,38px);
  margin:14px auto 0;flex-wrap:nowrap;}
...
.gw-medal{width:clamp(38px,5.4vw,52px);height:clamp(38px,5.4vw,52px);border-radius:50%;
  display:flex;align-items:center;justify-content:center;font-size:clamp(19px,3vw,27px);
...
.gw-fig{height:clamp(120px,21vh,210px);width:auto;object-fit:contain;
  filter:drop-shadow(0 10px 16px rgba(0,0,0,.5));animation:gwBob 2.6s ease-in-out infinite;}
.gw-r1 .gw-fig{height:clamp(140px,25vh,240px);}   /* champion stands tallest */
...
.gw-name{margin-top:8px;font-size:clamp(13px,1.9vw,18px);font-weight:800;color:#fff3d6;
...
.gw-foot{margin-top:16px;font-size:13px;font-weight:700;color:#ffd77a;
  text-shadow:0 2px 8px rgba(0,0,0,.8);}
.gw-close{margin:12px auto 0;cursor:pointer;border:2px solid rgba(255,255,255,.5);border-radius:999px;
...
@media(max-width:760px){
  .gw-podiums{gap:4px}
  .gw-bless{font-size:12px}
  .gw-slot{width:31vw}
  .gw-fig{height:clamp(96px,16vh,150px)}
  .gw-r1 .gw-fig{height:clamp(110px,19vh,170px)}
}
