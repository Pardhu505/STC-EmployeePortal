...
import { DEPARTMENT_DATA } from '@/data/mock';

// --- Start: AuthContext and Mock Data for Self-Contained Component ---
...
const findEmpDetailsByEmail = (email) => {
  if (!email) return { empCode: null, empName: null };
  for (const dept of Object.values(DEPARTMENT_DATA)) {
    for (const subDept of Object.values(dept)) {
      for (const employee of subDept) {
        if (employee["Email ID"] === email) {
          return {
            empCode: employee["Emp code"],
  ...
  return { empCode: null, empName: null };
};
...
const EMPAttendance = () => {
  ...
  useEffect(() => {
    if (user && user.email) {
      const empDetails = findEmpDetailsByEmail(user.email);
      setEmployeeDetails(empDetails);
      if (!empDetails.empCode) setError("Employee code not found for this user.");
    }
  }, [user]);
